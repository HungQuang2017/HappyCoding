...
( function(factory) {
		...
		$.extend(Plugin.prototype, {
			...
			_construct : function() {
				 ...
				 * Wrap the Select
				 */
				var label = this.$el.parent().find(".label-control");

				this.$container = this.$el.wrapAll('<div class="label-up ' + this.settings.comboClass + ' ' + this.settings.themeClass + '" />').parent();
				...
				this.$input = $('<input type="text"' + ( isMobile ? 'tabindex="-1"' : '') + (this.$el.prop("required") ? 'required' : '') + ' class="' + this.settings.inputClass + '" name="' + this.$el.data('name') + '">').appendTo(this.$container)
			...
			_updateInput : function() {
				...
				if (this.$el.val()) {
					...
					this.$input.val(text);
					this.$el.addClass("combo-selected");

				} else {

					this.$input.val('');
					this.$el.removeClass("combo-selected")
			...
			_blur : function() {
				...
				if (this.$el.val()) {
					...
					this.$container.parent().find(".field-error").hide();

				} else {
					this.$el.removeClass("invalid");
					if (this.$el.prop("required")) {
						this.$el.addClass("invalid");
						this.$container.parent().find(".field-error").show();
					}
				}
